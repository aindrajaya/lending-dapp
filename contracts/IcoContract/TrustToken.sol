/**
 * Directory: p2p/contracts/IcoContract/TrustToken.sol
 * Implements EIP20 token standard: https://github.com/ethereum/EIPs/blob/master/EIPS/eip-20.md
 */
// SPDX-License-Identifier: MIT
pragma solidity >=0.4.25 <0.7.0; // Solidity compiler version default v ^0.5.0

import "./EIP20Interface.sol";
import "../SafeMath.sol";

contract TrustToken is EIP20Interface {
  using SafeMath for uint256; //  validates if an arithmetic operation, to avoid overflow and underflow

  // Modifiers are code that can be run before and/ or after a functinol call, this modifier make sure the address that call this Trust Token is the same with the propsal management contract address
  ...
  mapping(address => uint256) public tokenBalances;               // token balance of trustees
  ...
  mapping(address => bool) public isTrustee;                      // administration of property in trust with a legal obligation to administer solely for the purpose
  ...
  function participate() external payable {
    ...
    if((contractEtherBalance.add(msg.value)) > goal){          // update allowedToAdd when goal is reached
      ...
      returnAmount = msg.value.sub(allowedToAdd);               // save the amount of ether that is to be returned
    }
    ...
    if(returnAmount > 0){
      msg.sender.transfer(returnAmount); //transfer ether eover limit back to sender
    }
  }
  ...
  */ 
  ...
  /**
    ...
    */
  function getICOParameters() public view returns (
    uint256 icoGoal, uint256 icoEtherBalance, bool isActive, uint256 totalTokenSupply, uint256 icoParticipantCount, string memory tokenSymbol, uint256 tokenBalanceUser, uint256 etherBalanceUser, string memory icoName, uint256 numDecimals, uint256 numTrustees
  ){
    icoGoal = goal;
  ...
  /**
    ...
    */
  function getEthereBalances() public view returns(uint256){
